...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting rejected orders cleanup...');
    ...
    for (const venue of venues || []) {
      ...
      if (timeDiff <= 5) { // Within 5 minutes of COB
        console.log(`Cleaning up rejected orders for ${venue.name} at COB ${cobTime}`);
        
        // Delete rejected orders older than 2 hours
        ...
        if (deleteError) {
          console.error(`Error deleting rejected orders for ${venue.name}:`, deleteError);
          continue;
        }
        ...
        console.log(`Deleted ${deletedCount} rejected orders for ${venue.name}`);
        ...
        if (deletedCount > 0) {
          ...
          if (analyticsError) {
            console.error(`Error deleting analytics for ${venue.name}:`, analyticsError);
          }
        }
      }
    }

    console.log(`Cleanup complete. Total deleted: ${totalDeleted} rejected orders`);

    return new Response(
      JSON.stringify({ 
        success: true, 
        deleted: totalDeleted,
        message: `Cleaned up ${totalDeleted} rejected orders`
      }),
